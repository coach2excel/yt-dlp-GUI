...
#youtube_executable =r"youtube-dl"  # 不用，下载速度受限，太慢
#youtube_executable =r"yt-dlp"  
noconfig = ' --ignore-config '   # Don't load any more configuration files
proxy = ' socks5://127.0.0.1:1080 ' # 加上本地代理(如果用默认配置，就不用加了)， 可在GUI上修改
is_proxy = True  # 默认使用默认代理

#yt_dlp_cmd = r'C:/Python310/Scripts/yt-dlp.exe'
# C:\Python310\Scripts\yt-dlp.conf            
#default_url = "https://www.youtube.com/watch?v=ZAeHq5K4TXA"
...
'''
yt_dlp_cmd = r'C:/Python310/Scripts/yt-dlp.exe' + noconfig
...
def runCommand(cmd, timeout=None, window=None):
    ...
    try:
        ...
        p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.DEVNULL)
        output = ''
        skip = False
...
def download_video_audio_and_subtitle(window, values):
    ...
    window.refresh()
    runCommand(command, window=window)
    print('Done')
...
def DownloadGUI():
    ...
    combobox = sg.Combo(values=['', ], size=(12, 1), key='-COMBO-SUBTITLES-')
    layout = [
        [sg.Text('请先安装(pip install yt-dlp) 详情参见： https://github.com/yt-dlp/yt-dlp#installation', size=(100, 1), font=('Any 12'))],
        [sg.Text('YouTube Link:'), sg.Input(default_text=default_url, size=(60, 1), key='link')],
        ...
        [sg.Output(size=(90, 20), font='Courier 12')],  # 输出信息
